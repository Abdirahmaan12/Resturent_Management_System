...
function register_user($conn){
    ...
    if(in_array($file_type, $allowedImages)){
     ...
     if(count($eroor_Array) <= 0){
        $query = "INSERT INTO `users`(`user_id`, `emp_id`, `username`, `password`, `image`, `type`)
        VALUES('$new_id', '$emname',  '$username',  MD5('$password'),  '$save_name', '$usertype')";
...
function get_total_user($conn){
    ...
    $data = array();
    $array_data = array();
   $query ="select count(user_id) as user  from users";
    $result = $conn->query($query);
...
function get_total_customers($conn){
    ...
    $data = array();
    $array_data = array();
   $query ="select count(customer_id) as customer  from customers";
    $result = $conn->query($query);
...
function get_user_list($conn){
    ...
    $data = array();
    $array_data = array();
   $query ="select u.user_id, e.fullname, u.username, u.image,u.type from users u JOIN employee e on u.emp_id=e.emp_id";
    $result = $conn->query($query);
...
function update_user($conn){
    ...
    if(!empty($_FILES['image']['tmp_name'])){
        ...
        if(in_array($file_type, $allowedImages)){
         ...
         if(count($eroor_Array) <= 0){
            $query = "UPDATE users set users.emp_id= '$emname', users.username= '$username', 
             password= MD5('$password'), users.type= '$usertype'  where users.user_id= '$update_id'";
